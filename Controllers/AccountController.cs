...
namespace API.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<ActionResult<AuthResponseDto>> Login(LoginDto loginDto)
        {
            ...
            _ = int.TryParse(_configuration.GetSection("JWTSetting").GetSection("RefreshTokenValidityIn").Value!,
        ...
        public async Task<ActionResult<AuthResponseDto>> Login(RefreshTokenDto refreshTokenDto)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            if (principal is null || user is null || user.RefreshToken != refreshTokenDto.RefreshToken ||
        ...
        private ClaimsPrincipal? GetPrincipalFromExpiredToken(string refreshToken)
        {
            var tokenParameters = new TokenValidationParameters
            {
                ...
                IssuerSigningKey =
                    new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_configuration.GetSection("JWTSetting").Value!)),
                ValidateLifetime = false
            };

            var tokenHandler = new JwtSecurityTokenHandler();
            var principal = tokenHandler.ValidateToken(refreshToken, tokenParameters, out SecurityToken securityToken);
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordDto forgotPasswordDto)
        {
            ...
            request.AddHeader("Authorization", "Bearer afc0d6b377174416440a3332f5e05bf2");
            request.AddJsonBody(new
            {
                from = new { email = "mailtrap@demomailtrap.com" },
                to = new[] { new { email = user.Email } },
                template_uuid = "6162c17c-002d-478d-83b6-45a31e03f89b",
            ...
            if (responses.IsSuccessful)
            {
                ...
                {
                    IsSuccess = true,
                    Message = "Password Reset Link is send in email. Please check your email."
                });
            }
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordDto resetPasswordDto)
        {
            ...
            if (user is null)
            {
                ...
                {
                    IsSuccess = false,
                    Message = "User does not exist with this user!"
                });
            }
